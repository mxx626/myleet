 ...
 * 811. Subdomain Visit Count
 ...
 level, "discuss.leetcode.com". When we visit a domain like "discuss.leetcode.com",
...
public class SubdomainVisitCount {
    ...
    private void parser(String domain, Map<String, Integer> map){
        String[] arr = domain.split(" ");
        int times = Integer.parseInt(arr[0]);

        map.put(arr[1], map.getOrDefault(arr[1], 0)+times);
        int first_dot = arr[1].indexOf(".");
        int second_dot = arr[1].lastIndexOf(".");
        if (first_dot!=-1){
            String key = arr[1].substring(first_dot+1);
            map.put(key, map.getOrDefault(key, 0)+times);
        }
        if (second_dot!=-1 && second_dot!=first_dot){
            String key1 = arr[1].substring(second_dot+1);
            map.put(key1, map.getOrDefault(key1, 0)+times);
        }
    }
}
