...
public class DesignLogStorageSystem {
    /**
     * You are given several logs that each log contains a unique id and timestamp. Timestamp
     * is a string that has the following format: Year:Month:Day:Hour:Minute:Second, for example,
     * 2017:01:01:23:59:59. All domains are zero-padded decimal numbers.
     ...
     you need to return all logs start from 2016:01:01:01 to 2017:01:01:23, where log 3 is left
     outside the range.
     Note:
    ...
    public List<Integer> retrieve(String s, String e, String gra) {
        ...
        for (int i : map.keySet()){
            String str = map.get(i).substring(0, end);
            if (str.compareTo(s.substring(0,end))>=0  && str.compareTo(e.substring(0,end))<=0){
                res.add(i);
            }
        }
        return res;
