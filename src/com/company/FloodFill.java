...
public class FloodFill {
    ...
    public int[][] floodFill(int[][] image, int sr, int sc, int newColor) {
        ...
        dfs(image, row, col, sr, sc, image[sr][sc], newColor, visit);
        return image;
    }
    private void dfs(int[][] m, int r, int c, int i, int j, int org, int color, boolean[][] visit){
        visit[i][j]=true;
        m[i][j]=color;
        for (int[] d : delta){
            int x = i+d[0];
            int y = j+d[1];
            if (x<0 || x>=r || y<0 || y>=c || visit[x][y] || m[x][y]!=org) continue;
            dfs(m, r, c, x, y, org, color, visit);
        }
    }
}
