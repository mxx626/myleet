...
public class ShoppingOffers {
    public int shoppingOffers(List<Integer> price, List<List<Integer>> special, List<Integer> needs) {
        return dfs(price, needs, special, 0);
    }
    private int dfs(List<Integer> price, List<Integer> needs, List<List<Integer>> special, int start){
        int total = buy(price, needs);
        for (int i=start; i<special.size(); ++i){
            List<Integer> pri = special.get(i);
            List<Integer> tmp = new ArrayList<Integer>();
            for (int j=0; j<needs.size(); ++j){
                if (needs.get(j)<pri.get(j)){
                    tmp = null;
                    break;
                }
                tmp.add(needs.get(j)-pri.get(j));
            }
            if (tmp!=null){
                total = Math.min(total, pri.get(pri.size()-1)+dfs(price, tmp, special, i));
            }
        }
        return total;
    }
