...
public class TeemoAttacking {
     ...
     * and the poisoning time duration per Teemo's attacking, you need to output the total time
    ...
    public int findPoisonedDuration(int[] timeSeries, int duration) {
        ...
        for (int i=1; i<timeSeries.length; ++i){
            t += timeSeries[i]-timeSeries[i-1]>=duration ? duration : timeSeries[i]-timeSeries[i-1];
        }
        return t;
    }
}
