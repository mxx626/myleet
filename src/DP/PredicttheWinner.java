...
public class PredicttheWinner {
    ...
    public boolean PredictTheWinner(int[] nums) {
        ...
        int res = helper(nums, 0, len-1, dp);
        return res*2>=sum;
    }
    public int helper(int[] nums, int start, int end, int[][] memo){
        ...
        int ret1 = nums[start]+Math.min(helper(nums, start+2, end, memo), helper(nums,start+1, end-1, memo));
        int ret2 = nums[end] + Math.min(helper(nums, start+1, end-1, memo), helper(nums, start, end-2, memo));
        memo[start][end]=Math.max(ret1, ret2);
