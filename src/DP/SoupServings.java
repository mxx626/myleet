 ...
 * 808. Soup Servings
 ...
 B will become empty first. So the total probability of A becoming empty first plus
 half the probability that A and B become empty at the same time, is 0.25 * (1 + 1 + 0.5 + 0) = 0.625.
...
public class SoupServings {
    private int[][] serve = {{100,0},{75,25},{50,50},{25,75}};
    ...
    private double helper(int A, int B, Double[][] memo){
        ...
        memo[A][B]=0.0;
        for (int [] s : serve){
            memo[A][B] += helper(A-s[0], B-s[1], memo);
        }
